...
namespace System.Collections.Concurrent
{
    ...
    /// <see cref="ConcurrentDictionary{TKey,TValue}"/> are thread-safe and may be used
    ...
    public class ConcurrentDictionary<TKey, TValue> : IDictionary<TKey, TValue>, IDictionary, IReadOnlyDictionary<TKey, TValue>
    {
        ...
        // NOTE: this is only used for compatibility reasons to serialize the comparer.
        ...
        internal ConcurrentDictionary(int concurrencyLevel, int capacity, bool growLockArray, IEqualityComparer<TKey> comparer)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(concurrencyLevel), string.Format("The concurrencyLevel argument must be positive."));
            }
            if (capacity < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(capacity), string.Format("The capacity argument must be greater than or equal to zero."));
            }
            if (comparer == null) throw new ArgumentNullException(nameof(comparer));
                ...
                lock (tables.m_locks[lockNo])
                {
                    ...
                    for (Node curr = tables.m_buckets[bucketNo]; curr != null; curr = curr.m_next)
                    {
                        ...
                        if (comparer.Equals(curr.m_key, key))
                        {
                            ...
                            if (prev == null)
                            {
                                FrameworkTraits.Volatile.Write(ref tables.m_buckets[bucketNo], curr.m_next);
                            }
                            else
                            {
        ...
        void ICollection<KeyValuePair<TKey, TValue>>.CopyTo(KeyValuePair<TKey, TValue>[] array, int index)
        {
            ...
            if (index < 0) throw new ArgumentOutOfRangeException(nameof(index), string.Format("The index argument is less than zero."));
        ...
        /// <see cref="ConcurrentDictionary{TKey,TValue}"/>.</returns>
        [SuppressMessage("Microsoft.Concurrency", "CA8001", Justification = "ConcurrencyCop just doesn't know about these locks")]
        ...
        public IEnumerator<KeyValuePair<TKey, TValue>> GetEnumerator()
        {
            ...
            for (int i = 0; i < buckets.Length; i++)
            {
                ...
                Node current = FrameworkTraits.Volatile.Read(ref buckets[i]);
        ...
        /// <see cref="ConcurrentDictionary{TKey,TValue}"/>.</value>
        ...
        private void GrowTable(Tables tables, IEqualityComparer<TKey> newComparer, bool regenerateHashKeys, int rehashCount)
        {
            ...
            try
            {
                ...
                // If this resize regenerated the hash-keys, increment the count
                if (regenerateHashKeys)
                {
        ...
        */
        ...
        private void OnSerializing(StreamingContext context)
        {
            ...
            m_comparer = tables.m_comparer ?? EqualityComparer<TKey>.Default;
        }
